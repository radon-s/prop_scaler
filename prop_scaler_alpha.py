...
class prop_scaler:
    def __init__(self, path, new_path, name, scale):
        ...
        self.path_qc = f'{path}\\{name}\\{name}.qc'
        self.path_qc_new = f'{path}\\{name}\\{name}_scaled_{scale.replace(".","_")}.qc'
        user = os.getlogin()
        self.path_bat = f'C:\\Users\\{user}\\Desktop\\modelscaler_compiler.bat'
    ...
    def model_name(self):
        ...
        if modelname != 'a':
            ...
            modelname_new_split = modelname_new.split(splitter, 1)[1]
            modelname_new_path = f'$modelname "{self.new_path}\\{modelname_new_split.split(splitter, 1)[1]}'
